...
echo "Signup User$TIMESTAMP@example.com"
STATUS=$(curl $URL/signup -d '{ "email" : "User'$TIMESTAMP'@example.com", "password" : "bad password" }' --write-out "%{http_code}\n" --silent --output response.json)
check_200_response $STATUS

echo "Signin User$TIMESTAMP@example.com"
STATUS=$(curl $URL/signin -d '{ "email" : "User'$TIMESTAMP'@example.com", "password" : "bad password" }' --write-out "%{http_code}\n" --silent --output response.json)
check_200_response $STATUS
USER_ACCESS_TOKEN=`cat response.json`

echo "GET $URL/tasks as User$TIMESTAMP@example.com"
STATUS=$(curl $URL/tasks -H 'Authorization: Bearer '$USER_ACCESS_TOKEN --write-out "%{http_code}\n" --silent --output response.json)
check_200_response STATUS

echo "POST $URL/tasks as User$TIMESTAMP@example.com"
...
echo "PUT $URL/tasks/$TASK_ID/notes/$NOTE_ID as User$TIMESTAMP@example.com"
...
echo "POST $URL/trackers as User$TIMESTAMP@example.com"
...
echo "PUT $URL/trackers/$TRACKER_ID as User$TIMESTAMP@example.com"
...
echo "GET $URL/tasks?format=html&from=2019-05-01 as User$TIMESTAMP@example.com"
...
echo "DELETE $URL/tasks/$TASK_ID/notes/$NOTE_ID as User$TIMESTAMP@example.com"
...
echo "DELETE $URL/tasks/$TASK_ID as User$TIMESTAMP@example.com"
STATUS=$(curl -X DELETE $URL/tasks/$TASK_ID -H 'Authorization: Bearer '$USER_ACCESS_TOKEN --write-out "%{http_code}\n" --silent --output response.json)
check_204_response $STATUS

echo "GET $URL/users as User$TIMESTAMP@example.com"
